...
const SUPERADMIN_AVATAR_BUCKET = "profile-uploads";
...
export const getSuperAdminProfileById = async (adminId) => {
  ...
  if (error || !data) {
    return { success: false, statusCode: 404, message: "Super Admin profile not found" };
  }
...
export const updateSuperAdminAvatarById = async (adminId, payload = {}) => {
  ...
  const storedPath = await uploadBase64ToSupabase(SUPERADMIN_AVATAR_BUCKET, avatarPath, avatarBase64);
...
export const updateSuperAdminProfileById = async (adminId, payload) => {
  const updates = {
    updated_at: new Date().toISOString(),
  };
  ...
  return getSuperAdminProfileById(adminId);
};